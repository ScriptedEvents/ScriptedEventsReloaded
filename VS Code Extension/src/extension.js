...
const SERLanguageCore = require('./ser-language-core.js');
const {
    findQuotedStringAtPosition,
    getMethodCallContext: resolveMethodCallContext,
    tokenizeSerExpression
} = require('./ser-editor-logic.js');
...
const VARIABLE_TYPE_NAMES = SERLanguageCore.VARIABLE_TYPE_NAMES;

function hasOwn(object, key) {
    return SERLanguageCore.hasOwn(object, key);
}

function getMethodCallContext(lineText, cursorCharacter) {
    return resolveMethodCallContext(lineText, cursorCharacter, SER_TRUTH_TABLE.methods);
}
...
function codeCompletions(position, typedName, returningOnly = false) {
    ...
    for (const [name, method] of Object.entries(SER_TRUTH_TABLE.methods || {})) {
        ...
        if (method.arguments?.length > 0) {
            ...
            item.command = {
                command: argumentHasImmediateCompletions(method.arguments[0])
                    ? 'editor.action.triggerSuggest'
                    : 'editor.action.triggerParameterHints',
                title: 'Show SER argument help'
            };
        } else {
            item.insertText = name;
        }
    ...
    if (!returningOnly) {
        for (const [name, keyword] of Object.entries(SER_TRUTH_TABLE.keywords || {})) {
            ...
            item.insertText = keyword.syntax && keyword.syntax !== name ? `${name} ` : name;
            item.filterText = name;
            item.range = range;
...
function flagCompletions(position, typedName) {
    ...
    return Object.entries(SER_TRUTH_TABLE.flags || {}).map(([name, flag]) => {
        ...
        if (name === 'OnEvent' || name === 'OnPMER' || name === 'OnUCR') {
            item.command = {
                command: 'editor.action.triggerSuggest',
                title: name === 'OnPMER'
                    ? 'Show ProjectMER events'
                    : name === 'OnUCR'
                        ? 'Show UCR events'
                        : 'Show SER events'
            };
...
function eventCompletions(position, typedName, source = 'ser') {
    const range = completionRange(position, typedName);
    const events = source === 'pmer'
        ? SER_TRUTH_TABLE.pmerEvents
        : source === 'ucr'
            ? SER_TRUTH_TABLE.ucrEvents
            : SER_TRUTH_TABLE.events;
    const eventDetails = source === 'pmer'
        ? SER_TRUTH_TABLE.pmerEventDetails
        : source === 'ucr'
            ? SER_TRUTH_TABLE.ucrEventDetails
            : SER_TRUTH_TABLE.eventDetails;
    return (events || []).map(eventName => {
        ...
        item.range = range;
        const sourceName = source === 'pmer' ? 'ProjectMER event' : source === 'ucr' ? 'UCR event' : 'SER event';
        item.detail = details?.group ? `${sourceName} · ${details.group}` : sourceName;
        if (details) {
...
async function provideCompletions(document, position) {
    ...
    const eventMatch = linePrefix.match(/^\s*!--\s+(OnEvent|OnPMER|OnUCR)\s+([A-Za-z0-9_]*)$/);
    if (eventMatch) {
        const source = eventMatch[1] === 'OnPMER' ? 'pmer' : eventMatch[1] === 'OnUCR' ? 'ucr' : 'ser';
        return eventCompletions(position, eventMatch[2], source);
    }
...
function visualEditorHtml(context, webview) {
    const editorPath = vscode.Uri.joinPath(context.extensionUri, 'out', 'visual-editor.html');
    const mediaPath = vscode.Uri.joinPath(context.extensionUri, 'out', 'media');
    ...
    return fs.readFileSync(editorPath.fsPath, 'utf8')
        .replace(
            '<head>',
            `<head><meta http-equiv="Content-Security-Policy" content="${csp}">`
        )
        .replaceAll('__SER_WEBVIEW_NONCE__', nonce)
        .replaceAll('__SER_WEBVIEW_MEDIA__', webview.asWebviewUri(mediaPath).toString());
}

async function saveVisualEditorFile(message) {
    const extension = message.filename?.endsWith('.json') ? 'json' : 'ser';
    const destination = await vscode.window.showSaveDialog({
        defaultUri: vscode.workspace.workspaceFolders?.[0]
            ? vscode.Uri.joinPath(vscode.workspace.workspaceFolders[0].uri, message.filename)
            : undefined,
        filters: extension === 'json'
            ? { 'SER block projects': ['json'] }
            : { 'SER scripts': ['ser'] },
        saveLabel: extension === 'json' ? 'Save block project' : 'Save SER script'
    });
    if (!destination) return;
    await vscode.workspace.fs.writeFile(destination, Buffer.from(String(message.text || ''), 'utf8'));
    await vscode.window.showTextDocument(destination, { preview: false });
}

function registerVisualEditor(context) {
    return vscode.commands.registerCommand('ser.openVisualEditor', () => {
        ...
        panel.webview.html = visualEditorHtml(context, panel.webview);
        panel.webview.onDidReceiveMessage(async message => {
            if (message?.type === 'saveFile') await saveVisualEditorFile(message);
        }, undefined, context.subscriptions);
    });
}

function updateSharedDiagnostics(document, collection) {
    if (document.languageId !== 'ser') return;
    const diagnostics = SERLanguageCore.validateGeneratedCode(document.getText()).map(item => {
        const lineIndex = Math.max(0, Math.min((item.line || 1) - 1, document.lineCount - 1));
        ...
        const diagnostic = new vscode.Diagnostic(line.range, item.message, severity);
        diagnostic.source = 'SER';
...
function registerSharedDiagnostics(context) {
    const collection = vscode.languages.createDiagnosticCollection('ser');
    const subscriptions = [
        collection,
        vscode.workspace.onDidOpenTextDocument(document => updateSharedDiagnostics(document, collection)),
        vscode.workspace.onDidChangeTextDocument(event => updateSharedDiagnostics(event.document, collection)),
        vscode.workspace.onDidCloseTextDocument(document => collection.delete(document.uri))
    ];
    for (const document of vscode.workspace.textDocuments) {
        updateSharedDiagnostics(document, collection);
    }
    context.subscriptions.push(...subscriptions);
}

function activate(context) {
    const visualEditorCommand = registerVisualEditor(context);
    registerSharedDiagnostics(context);
